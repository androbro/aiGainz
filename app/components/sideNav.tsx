...
export default function SideNav({ navClosed }: NavProps) {
    ...
    // const isAllowedToConfigure = role === UserRole.Configure;

    // const renderConfigurationSection = () => {
    //     if (isAllowedToConfigure) {
    //         return <NavSection name="Configurations" items={configurations} />;
    //     }
    //     return <></>;
    // };
    ...
    const [openGeneralSettings, setOpenGeneralSettings] = useState<boolean>(false);
    ...
    const Footer = () => (
        <div className="mt-auto flex align-items-center border-top-1 surface-border gap-3">
            <div>
                <a
                    onClick={() => {
    ...
    return (
        <>
            <SignOut visible={signOutVisible} onCloseModal={() => setSignOutVisible(false)} />
